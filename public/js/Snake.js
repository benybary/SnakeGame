export default class Snake {
  ...
  move() {
    ...
    if (this.apple.x === x && this.apple.y === y){
        // eaten the apple
        this.body.push(this.scene.add.rectangle(0,0,this.tileSize,this.tileSize, 0xfffff).setOrigin(0,0));
        this.positionApple();
    }
    ...
    if (this.body[0].x< 0 || 
     ...
     // deatch by eating own tail - headpos === any body
        let tail = this.body.slice(1);
        // if(tail.filter(s => s.x === this.body[0].x && s.y ===this.body[0].y).length){
        //     this.scene.scene.restart();
        // }
